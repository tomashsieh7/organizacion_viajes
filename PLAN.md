...
| Abierto/cerrado | Toda regla con variación prevista y toda dependencia externa van detrás de una interfaz, y los comportamientos nuevos se agregan como clases nuevas. Los puntos de extensión del MVP son `PoliticaResolucionOpciones` (P9), `PoliticaSuperposicion` (P10), `ProveedorRecorrido` (P20), `ProveedorAutenticacion` (P3), `BuscadorUbicaciones` (D8) y `EstrategiaDivision` (partes iguales y arbitraria). Los catálogos que pueden crecer, como categorías de gasto y monedas, son datos en tablas y no enumerados en el código (P16). |
...
| CU07, CU13 | `POST /api/viajes/:viajeId/propuestas/:propuestaId/confirmar` | Admin | Estado `PENDIENTE`; si es actividad, `PoliticaSuperposicion` contra las confirmadas (P10) y luego `PoliticaResolucionOpciones` sobre las demás opciones del grupo (P9) | 409 `TRANSICION_INVALIDA`, 409 `SUPERPOSICION_HORARIA` con los conflictos |
...
| RN-M6 | Se traza el recorrido entre ellas según ese orden, con líneas rectas en el MVP (P20). | `ProveedorRecorrido` (`RecorridoEnLineaRecta`) en `ConsultarMapa` y `MapaActividades` | `recorrido` |
...
| R3 — Subgrupos | Participantes de una actividad = quienes votaron a favor; ver solo actividades propias; canal de chat por actividad; gasto preseleccionado con esos participantes | `voto.valor = A_FAVOR` ya identifica a los participantes; las salas de Socket.IO pasan a `canal:<id>` y `mensaje` suma una columna `canal_id` nula para el chat general; `gasto_parte` ya admite cualquier subconjunto. Por la reserva del usuario sobre P9 y P10, este release agrega implementaciones nuevas de `PoliticaResolucionOpciones` y `PoliticaSuperposicion` para que opciones alternativas y actividades superpuestas convivan cuando no comparten participantes. |
| R4 — Mapa | Filtros por día o categoría, rutas y tiempos estimados, ubicación en tiempo real, lugares de interés | Otra implementación de `ProveedorRecorrido` para el trazado por calles; la ubicación en vivo usa la conexión Socket.IO; lugares de interés en una tabla nueva. |
...
|---|---|---|
| Otras formas de ingreso (teléfono, Google) | P3 | Nuevas implementaciones de `ProveedorAutenticacion` y nuevos valores de `credencial.tipo`. |
...
| P9 | Alternativas | `alternativa_de_id`; sin cadenas; solo sobre actividades pendientes; mismo control de superposición; confirmar una opción deniega las demás y denegar la original no afecta a las alternativas. Regla aislada en `PoliticaResolucionOpciones` por la reserva del usuario para el Release 3. |
| P10 | Superposición al confirmar | Se vuelve a controlar al confirmar, con `PoliticaSuperposicion`. |
...
| P20 | Recorrido | Líneas rectas en orden, detrás de `ProveedorRecorrido`. |
